...
if uploaded_file is not None:
    ...
    # 🔒 ขั้นตอนที่ 1: ตรวจสอบความถูกต้องของประเภทไฟล์ (Validation)
    with pdfplumber.open(uploaded_file) as pdf:
        ...
        if not first_page_text or "รายงานยานพาหนะ" not in first_page_text.replace(' ', ''):
            ...
            st.stop() # สั่งหยุดการทำงานทันทีหากไฟล์ผิดประเภท
            
    # ถ้ารอดการตรวจสอบมาได้ ให้เริ่มทำงานต่อ
    overdue_list = []
    total_records = 0
            ...
            if due_date:
                try:
                    ...
                    if expiry_date_object < current_date_core:
                        overdue_list.append({
                            # ลำดับที่ 1: ลำดับที่รันใหม่ (1, 2, 3...)
                            "ลำดับที่พบ": str(len(overdue_list) + 1), 
                            # ลำดับที่ 2: ลำดับในเอกสาร (1, 2, 3... ของคันนั้นๆ ใน PDF)
